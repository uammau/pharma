...
class prescribersAVLNode(object):
    ...
    def writetoFileReverseOrder(self, outputFile):
        if (self == None):
            return
        else:
            if (self.right != None):
                self.right.writetoFileReverseOrder(outputFile)
            
            # Writes to the file what is expected
            outputFile.write('\n' + self.key)
                
            if (self.left != None):
                self.left.writetoFileReverseOrder(outputFile)
                

    def writetoFileInOrder(self, outputFile):
        if (self == None):
            return
        else:
...
class prescribersAVL(object):
    ...
    def insert(self, k):
        ...
        """
        val = 1
        node = prescribersAVLNode(None, k)
    ...
    def delete(self, k):
        ...
        if node is self.root:
            pseudoroot = prescribersAVLNode(None, 0)
